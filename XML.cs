...
namespace ScriptStack
{
    public class XML : Model
    {
        ...
        public XML()
        {
            ...
            routines.Add(new Routine((Type)null, "xml.parse", (Type)null, "Erstelle ein XML Dokument (XDocument) aus einem String."));
            routines.Add(new Routine(typeof(string), "xml.string", (Type)null, "Gibt das XML (XDocument/XNode/XAttribute) als String zurück."));

            routines.Add(new Routine((Type)null, "xml.select", (Type)null, (Type)null, "Wählt einen(!) Knoten per XPath aus (XElement/XAttribute/XText) oder null."));
            routines.Add(new Routine(typeof(ScriptStack.Runtime.ArrayList), "xml.select_all", (Type)null, (Type)null, "Wählt mehrere Knoten per XPath aus (ArrayList von Nodes)."));

            routines.Add(new Routine(typeof(string), "xml.value", (Type)null, "Liest den String-Value eines Nodes (XElement/XAttribute/XText)."));
            routines.Add(new Routine(typeof(string), "xml.attr", (Type)null, (Type)null, "Liest ein Attribut (Name) von einem Element (oder null)."));
            routines.Add(new Routine(typeof(bool), "xml.has", (Type)null, (Type)null, "Prüft ob ein XPath ein Ergebnis liefert."));

            routines.Add(new Routine((Type)null, "xml.set", (Type)null, (Type)null, (Type)null, "Setzt den Value eines Elements/Attributes per XPath (erstes Match)."));

            // type checks
            routines.Add(new Routine(typeof(bool), "xml.is_elem", (Type)null, "True wenn Node ein XElement ist."));
            routines.Add(new Routine(typeof(bool), "xml.is_attr", (Type)null, "True wenn Node ein XAttribute ist."));

            // iterators
            routines.Add(new Routine((Type)null, "xml.iter", (Type)null, "Iterator über Child-Nodes (XDocument/XElement/XNode oder XML-String)."));
            routines.Add(new Routine((Type)null, "xml.iter_all", (Type)null, "Iterator über Descendants (Unter-Elemente) eines Elements."));
            routines.Add(new Routine((Type)null, "xml.iter_attr", (Type)null, "Iterator über Attribute eines Elements."));

            routines.Add(new Routine(typeof(bool), "xml.next", (Type)null, "Iterator: nächstes Element. true wenn vorhanden."));
            routines.Add(new Routine(typeof(string), "xml.name", (Type)null, "Iterator: Name (Element/Attribut) oder null."));
            routines.Add(new Routine(typeof(int), "xml.index", (Type)null, "Iterator: Index (0..n-1) oder -1 wenn kein current."));
            routines.Add(new Routine((Type)null, "xml.node", (Type)null, "Iterator: aktueller Node (XElement/XAttribute/XText/XNode)."));
            routines.Add(new Routine(typeof(string), "xml.value_it", (Type)null, "Iterator: Value des aktuellen Nodes (wie xml_value)."));
        ...
        public object Invoke(string routine, List<object> parameters)
        {
            ...
            if (routine == "xml.set")
            {
                ...
                if (root is XAttribute)
                    throw new InvalidOperationException("xml.set: root darf kein Attribut sein.");

                // Kontext fürs XPath: Root-Element
                XElement ctxElem = ToXPathContextElement(root);
                if (ctxElem == null)
                    throw new InvalidOperationException("xml.set: erwartet XDocument oder XElement (mit Root).");
        ...
        private static object CoerceToXmlObject(object o)
        {
            ...
            throw new InvalidOperationException("xml.iter erwartet XDocument/XElement/XAttribute/XText/XNode oder XML-String.");
        }
        ...
        private sealed class XmlIterator
        {
            ...
            private void Reset()
            {
                ...
                throw new InvalidOperationException("xml.iter: Unbekannter Root-Typ.");
            }
